    ...
    if args['add_hvac']
      case args['system_type']
        when "Ideal Air Loads"
          ...
          # setting damper position
          model.apply_multizone_vav_outdoor_air_sizing(args['template'])
      ...
      # todo- remove temp hvac code once fix is found for plant sizing issue in Miami
=begin
      hard_sized_flow_rate_m_s = 0.0014
      model.getCoolingTowerSingleSpeeds.each do |cooling_tower|
        cooling_tower.setDesignWaterFlowRate(hard_sized_flow_rate_m_s)
      end
      model.getCoolingTowerTwoSpeeds.each do |cooling_tower|
        cooling_tower.setDesignWaterFlowRate(hard_sized_flow_rate_m_s)
      end
